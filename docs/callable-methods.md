...
### Manual setup (without the Vite plugin)

If you cannot use the `agents/vite` plugin, you can configure the Babel transform yourself. Install the dependencies:
...
See the [Vite 8 migration guide](https://main.vite.dev/guide/migration#javascript-transforms-by-oxc) for more on OXC limitations and the Babel/SWC workaround pattern.

## TypeScript Configuration

Set `"target"` to `"ES2021"` or later in your `tsconfig.json`:
...
This ensures TypeScript emits decorator syntax that the Vite plugin (or Babel) can transform. Without it, decorators may be passed through as native syntax that the Workers runtime does not support.
